...
const App: React.FC = () => {
  ...
  // Determine which rows should be printed on the PDF
  // We trim the list to end at the lowest sphere that actually has data
  const printableSpheres = useMemo(() => {
    ...
    // Slice includes the row with data (index + 1)
    // If no data exists, it defaults to showing just the first row (0.00)
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pageElements.length; i++) {
        ...
        // CRITICAL FIX: "Unable to find element in cloned iframe"
        // Instead of asking html2canvas to find an element in a hidden tree,
        // we explicitly clone the node, make it visible on the main body, capture it, then destroy it.
        const clonedPage = originalPage.cloneNode(true) as HTMLElement;
        
        // Reset positioning on the clone to ensure it renders at 0,0 of the canvas
        clonedPage.style.position = 'fixed';
        clonedPage.style.top = '0';
        clonedPage.style.left = '0';
        clonedPage.style.zIndex = '-50'; // Behind the loading overlay but technically "visible" to the browser
        ...
        // Optional: wait a tick for layout
